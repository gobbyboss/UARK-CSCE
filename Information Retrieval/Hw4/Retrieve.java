...
public class Retrieve {
    ...
    public ResultList sortAccumulator()
    {
        ...
        for(int i: this.weightedIds)
        {
            ...
            ResultList newNode, curr, prev;
            if(size < 10)
            {
                if(head == null)
                {   
                    head = new ResultList(docId, weight);
                    tail = head;
                }
                else
                {
                    curr = head;
                    prev = null;
                    while((curr.next != null) && (weight < curr.getWeight()))
                    {
                        prev = curr;
                        curr = curr.next;
                    }
                    newNode = new ResultList(docId, weight);
                    if(prev == null)
                    {
                        newNode.next = head;
                        head = newNode;
    ...
    public void displayResults(ResultList head)
    {
        ResultList temp = head;
        int rank = 1;
        ...
        else
        {
            while(temp != null)
            {
                System.out.println("Rank: " + rank + "     Document Name: " + getMapName(temp.getDocId()) + "     Weight: " + temp.getWeight());
                rank++;
                temp = temp.next;
            }
        }
    ...
    public void run(Retrieve retriever)
    {
        ...
        ResultList head = retriever.sortAccumulator();
        retriever.displayResults(head);
    }
