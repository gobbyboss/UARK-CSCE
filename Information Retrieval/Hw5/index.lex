...
void writeMapRecord(ofstream &fpout, const string fileName) {    
    fpout << setw(FILENAME_SIZE) << fileName.substr(0, min((int)fileName.length(), FILENAME_SIZE)) << endl;
    num_records += 1;
}
...
void insertIntoHash(const char Str[]){
   ...
   {
      if ('A' <= Str[i] && Str[i] <= 'Z')
         downcased.push_back('a' + Str[i] - 'A');
      else
         downcased.push_back(Str[i]);
   } 

   if (downcased.length() != 1){

      totalTokens = totalTokens + 1;
      docTokens = docTokens += 1;

      int found = HighFreqHt.GetData(downcased);
      if (found == 1){
...
void ProcessDocumentHashtable(const char OutFile[])
{
   ...
   docId += 1; // Increasing the docId after writing to global hash table
}
...
/*
   const char* dictFile = dict.c_str();
   const char* postFile = post.c_str();
   GlobalHt.WriteToFiles(dictFile, postFile, docId);
*/
